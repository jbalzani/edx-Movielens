...
The edx dataset was briefly reviewed for NA values, outliers, and duplicates. It was found that there are no NA data points. After checking for duplicates, it was found that there are no duplicate values, as can be seen below.
...
From observing the vector, it can be seen that 0 is the lowest alpha. 
...
Below is a histogram of the predicted ratings, with a histogram of the actual ratings below it (Figures 3, 4).

```{r histograms of predicted vs. actual}
#create histogram of predictions
hist_pred <- hist(predicted_rating_final, main = "Figure 3: Histogram of Predicted Ratings")
hist_real <- hist(validation$rating, main = "Figure 4: Histogram of Actual Ratings")
```
