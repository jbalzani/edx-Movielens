...
```{r regularization - explore alpha}
...
```{r choose optimal alpha and rmse}
...
```{r calculate final predicted values}
...
The RMSE of the final model on the test set was `r rmse`, meaning that the predicted rating deviated from the actual rating by `r rmse` on average in the test set. Since the winners of the Netflix competition had an RMSE of .8713 on their winning algorithm, this is an acceptable RMSE for now (Koren, 2009).
